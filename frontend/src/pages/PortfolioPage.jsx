...
const PortfolioPage = () => {
  ...
  return (
    <section className="space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-semibold">Portfolio</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="glass-card rounded-3xl p-4 sm:p-6 flex flex-col justify-between">
              <div>
                ...
                <p className="mt-4 text-3xl sm:text-4xl font-bold text-sky-500">
            ...
            <div className="glass-card rounded-3xl p-4 sm:p-6 flex flex-col justify-between">
              <div>
                ...
                <p className={`mt-4 text-3xl sm:text-4xl font-bold ${metrics.totalUnrealizedPnL >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                  ...
                  <table className="w-full text-left text-sm text-[rgb(var(--text-secondary))] whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-[rgba(var(--border-subtle))]">
                      {holdingsList.map((position) => {
                        ...
                        return (
                          <tr key={position.symbol} className="hover:bg-[rgb(var(--bg-elevated))] transition">
                            <td className="py-4 px-4 font-semibold text-[rgb(var(--text-primary))]">
                              <div>
                                <p className="font-semibold">{position.symbol}</p>
                                <p className="text-xs text-[rgb(var(--text-muted))]">{position.name}</p>
                              </div>
                            </td>
                            ...
                            <td className="py-4 px-4 text-right font-medium text-[rgb(var(--text-primary))]">
                              ₹{position.currentValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
                            <td className={`py-4 px-4 text-right font-bold ${isProfit ? 'text-emerald-500' : 'text-rose-500'}`}>
                              {isProfit ? '+' : ''}
                    ...
                    return (
                      <div key={position.symbol} className="space-y-2">
                        <div className="flex justify-between text-sm font-semibold">
                          <span className="text-[rgb(var(--text-primary))]">{position.symbol}</span>
                          <span className="text-[rgb(var(--text-secondary))]">{percent}%</span>
                        </div>
                        <div className="h-2 w-full rounded-full bg-[rgb(var(--bg-elevated))] overflow-hidden">
                          <div
